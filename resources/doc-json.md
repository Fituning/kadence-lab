...
| Champ | Type | Description |
|---|---|---|
| `suggestedDayOfWeek` | enum | `mon` \| `tue` \| `wed` \| `thu` \| `fri` \| `sat` \| `sun` — jour indicatif au sein de la semaine, le plan n'étant pas suivi jour pour jour dans l'usage réel |
...
| `suggestedTerrainType` | string[] \| null | Optionnel, indicatif — un ou plusieurs parmi `road` \| `trail` \| `technical` \| `river` (même enum que `raceGoal.terrainType`). Ne reflète pas forcément ce que l'IA voudrait idéalement, mais ce à quoi l'utilisateur a accès ou ce qu'il souhaite éviter (ex : `["road", "trail"]` pour écarter le technique par prudence). |
...
Champs construits par l'app pour le prompt — colonne "Source" = d'où vient la valeur côté BDD (`users`, sauf mention contraire). Un champ sans source BDD est saisi/calculé ponctuellement à la génération, jamais persisté.

| Champ | Type | Source | Description |
|---|---|---|---|
| `name` | string | `users.first_name` | Prénom |
| `level` | enum | `users.level` | `beginner` \| `intermediate` \| `advanced` — niveau perçu/déclaré |
| `weeklyAvailability` | string[] | `users.preferred_weekly_availability` | Un ou plusieurs parmi `mon` \| `tue` \| `wed` \| `thu` \| `fri` \| `sat` \| `sun` — mêmes codes que `Session.suggestedDayOfWeek` |
| `initialAssessment` | object | *(aucune table dédiée pour l'instant, cf §7)* | Résultat du bilan `test_zones` — voir ci-dessous |
| `logisticalConstraints` | string \| null | `users.logistical_constraints` | Contraintes pratiques (accès au dénivelé, horaires serrés, etc.) |
| `injuryOrPainConcerns` | string \| null | `users.injury_or_pain_concerns` | Douleurs ou gênes actuelles (ex : "genoux sensibles") — l'IA en tient compte pour une progression de charge plus prudente |
| `plannedLifeEvents` | string \| null | `users.planned_life_events` | Événements de vie prévus pendant la préparation susceptibles d'affecter la récupération — l'IA peut positionner une semaine plus légère autour de cette période plutôt que de le découvrir après coup via un mauvais ressenti |

**Plus de champ `age`** : la v8 du schéma BDD stocke `users.date_of_birth` (plus précis, réutilisable ailleurs) au lieu d'un `age` brut. L'app calcule l'âge elle-même à partir de cette date juste avant de construire le prompt, si le contexte en a besoin — `age` n'existe donc plus comme champ direct de `athlete`.
...
| `terrainType` | string[] \| null | Optionnel — description factuelle du parcours réel, un ou plusieurs parmi `road` \| `trail` \| `technical` \| `river` (même enum que `Session.suggestedTerrainType`, sens différent : ici c'est ce que la course traverse réellement, pas une suggestion) |
| `startTime` | string \| null | Optionnel — heure de départ prévue |
| `targetTime` | int \| null | Optionnel — temps visé pour compléter l'épreuve |
...
## 10. Notes d'implémentation (BDD)

Détail complet dans `resources/doc-database.md` (tables, enums, points d'attention Zod). Résumé des décisions structurantes prises côté schéma Drizzle, pour ne pas les reperdre :

- **Ids** : jamais fournis par l'IA (rappel section 1). Générés en `uuid` app-side pour `Program`, `Race` et `Session` (ressources accédées directement par id) ; `Week` reste en `serial` (toujours consultée en cascade via son programme, pas de route dédiée).
- **Enums en anglais, alignés sur la BDD** : toutes les valeurs d'enum du JSON (`suggestedDayOfWeek`, `terrainType`/`suggestedTerrainType`, `athlete.level`, etc.) sont désormais en anglais et correspondent **exactement** aux valeurs des enums Postgres (`shared/database/enums.ts`) — l'IA doit les émettre telles quelles, aucune conversion FR → EN n'est faite à l'import. Zod valide directement contre ces enums anglais.
- **`scheduledDate`** : absente du JSON par design (section 3), mais matérialisée en colonne sur `sessions` côté BDD pour permettre le tri sans recomposer la date à chaque lecture. À recalculer explicitement si `Program.startDate` est modifiée après génération.